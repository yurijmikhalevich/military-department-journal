...
QSqlQuery BaseWidget::query;

BaseWidget::BaseWidget(QWidget *parent) :
    QWidget(parent)
{
    view = new SteroidsView(this);
    if (!query.driver()->isOpen()) { // initialize our common QSqlQuery object, if it not yet initialized
        query = QSqlQuery();
    }
}
...
bool BaseWidget::insertRecord(const QVariantMap record)
{
    ...
    columnNames += ")";
    placeholdersPlace += ")";
    if (!query.prepare(QString("INSERT INTO %1 %2 VALUES %3").arg(tableName, columnNames, placeholdersPlace))) {
